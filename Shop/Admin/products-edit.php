...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE products SET cat_id=%s, sup_id=%s, pro_name=%s, price=%s, quantity=%s, details=%s, images=%s WHERE pro_id=%s",
...
$colname_products = "-1";
if (isset($_GET['id'])) {
  $colname_products = $_GET['id'];
}
mysql_select_db($database_mydbshop, $mydbshop);
$query_products = sprintf("SELECT * FROM products WHERE pro_id = %s", GetSQLValueString($colname_products, "int"));
  ...
  <div class="row content">
    ...
    <form method="post" name="form1" action="<?php echo $editFormAction; ?>" enctype="multipart/form-data">
      <p>
          ...
          <input name="pro_name" class="w3-input" type="text" placeholder="กรุณากรอกชื่อรายการสินค้า" required value="<?php echo $row_products['pro_name']; ?>"></p>
          <p>
          <label style="float:left">หมวดหมู่สินค้า</label>
            <select name="cat_id" class="w3-input" >
			  ...
			  <?php do { if($row_products['cat_id']==$row_categories[cat_id]){?>
              <option value="<?php echo $row_categories['cat_id']; ?>" selected><?php echo $row_categories['cat_name']; ?></option>
              <?php }else{?>
        ...
        <p>
          <label style="float:left">ผู้จัดส่งสินค้า</label>
            <select name="sup_id" class="w3-input" >
                ...
                <?php do { if($row_products['sup_id']==$row_suppliers['sup_id']){?>
              <option value="<?php echo $row_suppliers['sup_id']; ?>" selected><?php echo $row_suppliers['sup_name']; ?></option>
              <?php }else{?>
          ...
          <input name="price" class="w3-input" type="text" placeholder="กรุณากรอกราคาสินค้า" required value="<?php echo $row_products['price']; ?>"></p>
           <p>
          <label style="float:left">จำนวนสินค้า</label>
          <input name="quantity" class="w3-input" type="text" placeholder="กรุณากรอกจำนวนสินค้า" required value="<?php echo $row_products['quantity']; ?>"></p>
           <p>
          <label style="float:left">รายละเอียดสินค้า</label>
          <textarea name="details" class="w3-input" type="text" placeholder="กรุณากรอกรายละเอียดสินค้า" ><?php echo $row_products['details']; ?></textarea></p>
          <p>
          <label style="float:left">รูปภาพสินค้า</label>
          <input name="images" class="w3-input" type="file" placeholder="กรุณากรอกรายละเอียดสินค้า" value="<?php echo $row_products['images']; ?>"></p>
      ...
      <input type="hidden" name="pro_id" value="<?php echo $row_products['pro_id']; ?>">
    </form>
