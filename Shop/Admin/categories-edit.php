...
if (isset($_GET['id'])) {
  $colname_categories = $_GET['id'];
}
mysql_select_db($database_mydbshop, $mydbshop);
$query_categories = sprintf("SELECT * FROM categories WHERE cat_id = %s", GetSQLValueString($colname_categories, "int"));
  ...
  <div class="row content">
    ...
    <div class="col-sm-8 text-center">
      ...
      <form action="<?php echo $editFormAction; ?>" method="POST" name="insert" class="w3-container">
          <p>
          <label style="float:left">ชื่อหมวดหมู่</label>
          <input name="cat_name" class="w3-input" type="text" placeholder="กรุณากรอกชื่อหมวดหมู่" required value="<?php echo $row_categories['cat_name']; ?>"></p>
          <p hidden="hidden">
          <label style="float:left">ชื่อหมวดหมู่</label>
          <input name="cat_id" class="w3-input" type="text" placeholder="กรุณากรอกชื่อหมวดหมู่" value="<?php echo $row_categories['cat_id']; ?>"></p>
          <button type="submit" class="w3-btn w3-green">บันทึก</button><button type="button" class="w3-btn w3-red" onClick="window.location.href='categories.php'">ยกเลิก</button>
