...
{
	$email = $_POST['email'];
	if($_POST['notify']=='notify'){
		$sql = "SELECT COUNT(*) FROM news WHERE email = '$email'";
					...
					if($data[0] != 0) {
						...
						$sql = "INSERT INTO news VALUES('$email', CURRENT_DATE())";
						mysqli_query($con, $sql);
						echo "<script>alert('Subscribe to promotion completed');</script>";
						$class = "";
					}
	}else{
		$sql = "DELETE FROM news WHERE email = '$email'";
					echo "<script>alert('Cancel promotion completed');</script>";
					mysqli_query($con, $sql);
	}
}
?>
...
<div class="form-group">
					    <label class="info-title" for="exampleInputEmail1">Email Address <span>*</span></label>
			 <input type="email" name="email" class="form-control unicase-form-control text-input" id="exampleInputEmail1" value="<?php echo $row['email'];?>">
					  </div>						
<div class="form-group">
                        	<input type="radio" class="radio-inline" name="notify" checked value="notify">
                            <label class="info-title">Subscribe to promotion</label>
                        </div>
                        <div class="form-group">
                        	<input type="radio" class="radio-inline" name="notify" value="unnotify">
                            <label class="info-title">Cancel promotion</label>
                        </div>
					  <button type="submit" name="notification" class="btn-upper btn btn-primary checkout-page-button">Save </button>
					</form> 
